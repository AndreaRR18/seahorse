export const CONTACT_INFO = {
  phone: '+39 123 456 7890',
  phoneDisplay: '+39 123 456 7890',
  email: 'info@cavallucciomarino.com',
  whatsapp: 'https://wa.me/391234567890',
  address: {
    street: 'Viale Calabria, 2B',
    city: 'Lodi',
    province: 'LO',
    zip: '26900',
    country: 'Italia'
  }
};

export const SCHEDULE = {
  days: 'Lunedì - Venerdì',
  hours: '8:00 - 17:30',
  note: 'Orari flessibili disponibili su richiesta',
  closures: 'Chiuso nei giorni festivi e nel mese di agosto'
};

export const SOCIAL_LINKS = {
  whatsapp: 'https://wa.me/391234567890',
  facebook: '', // Add if available
  instagram: '' // Add if available
};

export const SITE_INFO = {
  name: 'Cavalluccio Marino di Serena',
  tagline: 'Un nido accogliente dove ogni bambino è speciale',
  description: 'Asilo nido familiare a Lodi con cura personalizzata, attività educative e orari flessibili.',
  year: new Date().getFullYear()
};

export const NAV_ITEMS = [
  { id: 'home', label: 'Home' },
  { id: 'about', label: 'Chi Siamo' },
  { id: 'services', label: 'Servizi' },
  { id: 'schedule', label: 'Orari' },
  { id: 'contact', label: 'Contatti' }
];

export const VALUES = [
  {
    title: 'Attenzione Personalizzata',
    description: 'Ogni bambino riceve cure individuali e un\'attenzione dedicata ai suoi bisogni unici.'
  },
  {
    title: 'Flessibilità',
    description: 'Orari flessibili per le famiglie moderne, con ingressi e uscite personalizzate.'
  },
  {
    title: 'Esperienza',
    description: 'Anni di esperienza nell\'educazione infantile con un approccio professionale e caloroso.'
  },
  {
    title: 'Ambiente Accogliente',
    description: 'Uno spazio caldo e familiare dove i bambini si sentono a casa.'
  }
];

export const SERVICES = [
  {
    title: 'Cura Personalizzata',
    description: 'Attenzione individuale per ogni bambino basata su età, personalità e bisogni specifici.',
    highlights: [
      'Rapporto educatore-bambino ottimale',
      'Programmi adattati all\'età',
      'Comunicazione quotidiana con i genitori'
    ],
    image: 'https://placehold.co/800x600/ADD8E6/333?text=Cura+Personalizzata'
  },
  {
    title: 'Attività Educative',
    description: 'Giochi e attività creative per stimolare la crescita cognitiva, emotiva e sociale.',
    highlights: [
      'Attività artistiche e creative',
      'Giochi sensoriali e motori',
      'Lettura e musica quotidiana'
    ],
    image: 'https://placehold.co/800x600/98FF98/333?text=Attivita+Educative'
  },
  {
    title: 'Orari Flessibili',
    description: 'Adattiamo gli orari alle vostre esigenze, con pacchetti personalizzati e servizio estivo.',
    highlights: [
      'Ingressi e uscite personalizzate',
      'Pacchetti part-time disponibili',
      'Servizio estivo'
    ],
    image: 'https://placehold.co/800x600/FFB6C1/333?text=Orari+Flessibili'
  }
];
